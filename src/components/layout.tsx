...
export const PageLayout = (props: PropsWithChildren) => {
  ...
  const { x } = useElementPosition(userButtonRef);
      ...
      <div className="flex w-full max-w-4xl flex-row">
        <nav className="sticky top-0 h-screen flex-1 items-start justify-start border-r border-slate-50">
          <ul className="flex flex-col gap-1 p-2 text-lg">
            <li ref={userButtonRef} className="rounded-lg p-2 transition-all hover:bg-neutral-900">
              <UserButton
                showName={width > 900}
                afterSignOutUrl="/"
                appearance={{
                  elements: {
                    ...
                    userButtonPopoverCard: { left: `${x}px !important`, borderRadius: '0.5rem' },
                  },
                }}
              />
            </li>
